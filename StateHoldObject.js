...
export default function StateHoldObject()
{
    ...
    const changeDemo=()=>{setCollege((prevState)=>{return{...prevState,year:2024}})}
    const changeDemo1=()=>{setCollege((prevState)=>{return{...prevState,cname:"SKCET"}})}
    return(
        <div>
            <h1>College ID is {college.cid}</h1>
            <h1>College name is {college.cname}</h1>
            <button onClick={changeDemo1}>Change Name!</button>
